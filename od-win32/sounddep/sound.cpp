...
// Forward declarations for Layer 3 functions
static bool init_audio_wasapi_layer3(struct sound_dp *s, int sampleRate, int channels, int bits);
static void close_audio_wasapi_layer3(void);
...
#ifdef USE_AUDIO_LAYER2
	// If Layer 3 is active, don't use pullbuffer (Layer 3 pulls from Layer 2 directly)
	if (g_audioLayer2 && g_audioWASAPILayer3) {
		// ДИАГНОСТИКА
		static int skipCount = 0;
		skipCount++;
		if (skipCount % 100 == 0) {
			write_log(_T("DEBUG finish_sound_buffer_pull: SKIPPED (Layer3 active), count=%d\n"), skipCount);
		}
		return;  // Layer 3 handles pull via OnPullEvent
	}
#endif
...
public:
	...
	HRESULT STDMETHODCALLTYPE OnDefaultDeviceChanged(EDataFlow flow, ERole role, LPCWSTR pwstrDeviceId)
	{
		...
		if (sound_devices[s->index]->alname != NULL && 
			sound_devices[s->index]->alname[0] != '\0') {
			return S_OK;
		}
...
//	sd->sndbufsize = sd->sndbufsize * 2 / 3;
	...
	if (s->pullmode) {
		...
		if (sd->sndbufsize > v)
			sd->sndbufsize = v;
		s->wasapigoodsize = (s->bufferFrameCount * 3) / 2;
	}
    ...
    // Initialize Audio Layer 3 (WASAPI backend) if Layer 2 exists
#ifdef USE_AUDIO_LAYER2
    if (g_audioLayer2 && s->pullmode) {
        // Calculate bits per sample from samplesize
        int bits = (sd->samplesize / sd->channels) * 8;
		write_log(_T("AUDIO: Initializing Layer3 (freq=%d, ch=%d, bits=%d)...\n"),
			sd->freq, sd->channels, bits);
        init_audio_wasapi_layer3(s, sd->freq, sd->channels, bits);
    }
#endif
...
// Инициализация Audio Layer 2
static bool init_audio_layer2(void)
{
    write_log(_T("DEBUG: init_audio_layer2 CALLED\n"));
    
    // ЗАЩИТА: если уже инициализирован, не пересоздавать!
    if (g_audioLayer2) {
        write_log(_T("DEBUG: Layer2 already exists, checking initialized state\n"));
        if (g_audioLayer2->IsInitialized()) {
            write_log(_T("DEBUG: Layer2 already initialized, skipping\n"));
            return true;
        }
        write_log(_T("DEBUG: Cleaning up existing but uninitialized Layer2\n"));
        g_audioLayer2->Shutdown();
        delete g_audioLayer2;
        g_audioLayer2 = nullptr;
    }
    
    // Create new instance
    write_log(_T("DEBUG: Creating new AudioLayer2\n"));
    g_audioLayer2 = new AudioLayer2();
    if (!g_audioLayer2) {
        write_log(_T("AUDIO: Failed to allocate AudioLayer2\n"));
        return false;
    }
    write_log(_T("DEBUG: AudioLayer2 allocated at %p\n"), g_audioLayer2);
    
    // Configure
    AudioLayer2Config config = {};
    config.targetSampleRate = 48000;  // Fixed for Phase 1
    config.channels = 2;              // Stereo
    config.ringBufferFrames = 16384;   // ~340ms @ 48kHz to handle bursts
    
    write_log(_T("DEBUG: Config: rate=%d, ch=%d, frames=%d (NextPowerOf2 = %d)\n"),
              config.targetSampleRate, config.channels, config.ringBufferFrames, 
              (1 << (32 - __lzcnt(config.ringBufferFrames - 1)))); // Approximation of NextPowerOf2 for log
    
    // Initialize
    write_log(_T("DEBUG: Calling Initialize...\n"));
    if (!g_audioLayer2->Initialize(config)) {
        write_log(_T("AUDIO: AudioLayer2 initialization failed\n"));
        delete g_audioLayer2;
        g_audioLayer2 = nullptr;
        return false;
    }
    
    write_log(_T("AUDIO: Layer2 initialized successfully\n"));
    return true;
}

	// Закрытие Audio Layer 2
static void close_audio_layer2(void)
{
	if (g_audioLayer2) {
		write_log(_T("AUDIO: Shutting down Layer2\n"));
		g_audioLayer2->Shutdown();
		delete g_audioLayer2;
		g_audioLayer2 = nullptr;
	}
	...
	// Инициализация Audio Layer 3 (WASAPI backend)
static bool init_audio_wasapi_layer3(struct sound_dp *s, int sampleRate, int channels, int bits)
{
    if (!s || !s->pDevice || !s->pAudioClient || !s->pRenderClient) {
        write_log(_T("WASAPI Layer3: Cannot initialize - missing WASAPI interfaces\n"));
        return false;
    }
    
    // Cleanup existing instance
    if (g_audioWASAPILayer3) {
        g_audioWASAPILayer3->Shutdown();
        delete g_audioWASAPILayer3;
        g_audioWASAPILayer3 = nullptr;
    }
    
    // Only initialize if Layer 2 exists
    if (!g_audioLayer2) {
        write_log(_T("WASAPI Layer3: Layer 2 not initialized, skipping Layer 3\n"));
        return false;
    }
    
    // Create Layer 3 instance
    g_audioWASAPILayer3 = new AudioWASAPILayer3();
    if (!g_audioWASAPILayer3) {
        write_log(_T("WASAPI Layer3: Failed to allocate\n"));
        return false;
    }
    
    // Initialize with WASAPI interfaces
    if (!g_audioWASAPILayer3->Initialize(
            s->pDevice,
            s->pAudioClient,
            s->pRenderClient,
            sampleRate,
            ...
            s->bufferFrameCount))
    {
        write_log(_T("WASAPI Layer3: Initialization failed\n"));
        delete g_audioWASAPILayer3;
        g_audioWASAPILayer3 = nullptr;
        return false;
    }
    
    write_log(_T("WASAPI Layer3: Initialized successfully\n"));
    return true;
}
...
{
    if (g_audioWASAPILayer3) {
        write_log(_T("WASAPI Layer3: Shutting down\n"));
        g_audioWASAPILayer3->Shutdown();
        delete g_audioWASAPILayer3;
        g_audioWASAPILayer3 = nullptr;
    }
...
static int open_sound (void)
{
	...
	// Initialize Audio Layer 2 FIRST (before opening device)
#ifdef USE_AUDIO_LAYER2
	write_log(_T("AUDIO: Initializing Layer2 before opening sound device...\n"));
	if (!init_audio_layer2()) {
		write_log(_T("AUDIO: Layer2 initialization failed\n"));
		// Continue anyway - Layer2/3 are optional enhancements
	}
#endif
...
void close_sound (void)
{
    ...
    // Shutdown Audio Layer 3
    close_audio_wasapi_layer3();
	
	// Shutdown Audio Layer 2
	close_audio_layer2();
...
int init_sound (void)
{
	...
	// Audio Layer 2 is now initialized inside open_sound()
	/*
	if (!init_audio_layer2()) {
		write_log(_T("AUDIO: Layer2 init failed, falling back to legacy audio\n"));
	}
	*/
...
static bool finish_sound_buffer_wasapi_pull_do(struct sound_data *sd)
{
    ...
    if (g_audioLayer2 && g_audioWASAPILayer3) {
        ...
        pullCount++;
        if (pullCount % 50 == 0) {
            float fill = g_audioLayer2->GetBufferFillPercent();
            write_log(_T("DEBUG WASAPI pull: count=%d, Layer2 fill=%.1f%%\n"), 
                    pullCount, fill * 100.0f);
        }
        return g_audioWASAPILayer3->OnPullEvent(g_audioLayer2);
    }
#endif
    
    // Fallback to legacy code if Layer 2/3 not available
...
#ifdef USE_AUDIO_LAYER2
		// When Layer 2/3 are active, we must always return at least 1 
		// to allow audio_finish_pull to handle the WASAPI pull event.
		// If we return 0, the event is ignored, which can lead to deadlocks.
		if (g_audioLayer2 && g_audioWASAPILayer3) {
			return 1;
		}
#endif
		
		// Legacy path for old pullbuffer
		if (s->pullbufferlen > 0) {
			cnt++;
...
void finish_sound_buffer (void)
{
	...
	// НОВОЕ: Layer 2 integration
    write_log(_T("DEBUG: REACHED Layer2 integration point, have_sound=%d, bufsize=%d\n"), have_sound, bufsize);

    #ifdef USE_AUDIO_LAYER2
    {
        AudioLayer2* layer2Ptr = g_audioLayer2;
        if (layer2Ptr) {
            int frameCount = bufsize / (2 * sizeof(uae_u16));
            
            // Use hardware-native constants (unaffected by drift correction)
            extern float sampler_evtime;
            
            layer2Ptr->ProcessFromPaula(
                (int16_t*)paula_sndbuffer,
                frameCount,
                scaled_sample_evtime_orig,
                (double)sampler_evtime
            );
        }
    }
    #else
    static int useLayerErrLogCount = 0;
    if (useLayerErrLogCount < 100) {
        write_log(_T("ERROR: USE_AUDIO_LAYER2 not defined in finish_sound_buffer!\n"));
        useLayerErrLogCount++;
    }
    #endif
